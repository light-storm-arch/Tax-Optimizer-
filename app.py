...
if run_opt or run_zero:
    ...
    if scenario_enabled:
        ...
        alt_household = HouseholdConfig(**{**household.__dict__, "social_security_age_1": int(alt_ss_age_1), "social_security_age_2": int(alt_ss_age_2), "death_age_1": int(alt_death_age_1), "death_age_2": int(alt_death_age_2)})
        alt_cfg = ProjectionConfig(**{**cfg.__dict__, "annual_return": float(alt_return), "inflation": float(alt_infl)})

        if run_zero:
            alt_result = project_plan(alt_household, alt_cfg, {alt_household.start_year + i: 0.0 for i in range(alt_cfg.horizon_years)})
        else:
    ...
    if not chart_df.empty:
        ...
        line1 = px.line(balances, x="year", y="balance", color="scenario", facet_col="account", title="Account Balances by Year")
